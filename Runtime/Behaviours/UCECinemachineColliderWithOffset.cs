...
namespace UCE.Runtime.Behaviours
{
    ...
    /// <summary>
    /// 镜头 Collider 扩展，用于镜头与目标之间有障碍物时，将镜头拉近避免目标被遮挡
    /// 在原本 CinemachineCollider 进行了部分修改，使其支持与 Offset 配合使用
    ///
    ...
    [ExecuteInEditMode]
#endif
    [RequireComponent(typeof(UCECinemachineCameraOffset))]
    [DisallowMultipleComponent]
    public class UCECinemachineColliderWithOffset : CinemachineExtension
    {
        /// <summary>Objects on these layers will be detected.</summary>
        [Header("Obstacle Detection")] [Tooltip("Objects on these layers will be detected")]
        ...
        private UCECinemachineCameraOffset _cinemachineCameraOffset;
        ...
        protected override void Awake()
        {
            ...
            _cinemachineCameraOffset = GetComponent<UCECinemachineCameraOffset>();
        }
            ...
            CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
        {
            ...
            if (stage == CinemachineCore.Stage.Body)
            {
                if (m_AvoidObstacles)
                {
                    ...
                    // check offset extension will enable
                    if (_cinemachineCameraOffset)
                        _cinemachineCameraOffset.m_Active = displacement.sqrMagnitude <= Mathf.Epsilon;
                }
            }
